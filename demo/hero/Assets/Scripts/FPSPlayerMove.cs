...
public class FPSPlayerMove : MonoBehaviour {
	...
	void Start () {
		...
		groundedRaycastDistance = 3f;/* capsuleCollider.height / 2 + 0.01f;*/	//计算向地面发射射线的射线长度
	}
	...
	void Jump(bool isGround)
	{
		...
		if (CrossPlatformInputManager.GetButtonDown("Jump") /*Input.GetKeyDown(KeyCode.Space)*/ && isGround &&(GameManager.gm == null || GameManager.gm.gameState == GameManager.GameState.Playing) )
		{
			
			//给玩家刚体组件添加向上的作用力，以改变玩家的运动速度，改变值为jumpVelocity
			rigid.AddForce(Vector3.up * jumpVelocity, ForceMode.VelocityChange);
			Debug.Log("tiaotaiaotoatoatoaotaotoat");
		}
	}
